...
function MenuItem(props: MenuItemProps) {
    ...
    if (zone === "link") {
        return (
            <a href={params[0]} target={params[1]}>
                {title}
            </a>
        );
    }
...
function Nav4TrunkItem(props: Nav4TrunkItemProps) {
    if (props.clientSideRouting) {
        ...
        const className =
            targetUrl.pathname === window.location.pathname ? "active" : "";
        return (
...
export function Nav4Trunk(props: Nav4Props) {
    let navElements = props.trunk.map((itemProps, idx) => (
        <Nav4TrunkItem key={idx} {...itemProps}></Nav4TrunkItem>
    ));
    return (
        <div className="nav4-trunk">
...
export function Nav4Menu(props: Nav4Props) {
    ...
    let actionElements = actions.map((action, idx) => (
        <MenuItem key={idx} action={action} />
    ));
    return (
        <div className="dropdown">
            <a className="broad" href="#">
                {text}
                <span className="icon">{props.menu.icon}</span>
            </a>
            <div className="dropdown-content">{actionElements}</div>
        </div>
    );
}
...
export function Nav4(props: Nav4Props) {
    return (
        <div className="zenux-dashboard nav4">
